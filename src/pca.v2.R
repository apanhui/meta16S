...
Usage: Rscript pca.R <matrix file> [start,end] [group file]
...
Note: 1. the matrix file must be seperated by <TAB>
      ...
      5. [group file] contains two fields, first is sample name, second is group names, 
         and the second field's names must be 'Group'
\n"
  cat(usage)
  quit("no")
}
...
# read the data
data = read.table(args[1],header=T,row.names=1,sep="\t",check.names=F,comment.char="")

outname = unlist(strsplit(args[1],"/"))
outname = outname[length(outname)]
...
write.table(cbind(ID=rownames(rotation),rotation),file=paste(outname,".PCA.component.xls",sep=""),sep="\t", quote=FALSE,row.names=FALSE)

x = data.pca$x
write.table(cbind(Samples=rownames(x),x),file=paste(outname,".PCA.samples.xls",sep=""),sep="\t",quote=FALSE,row.names=FALSE)

print(max(x[,1]))

cat("[LOG] the result of PCA was saved into files:\n\t",
  paste(outname,"PCA.samples.xls",sep=""),"and\n\t",paste(outname,".PCA.component.xls",sep=""),"\n")
...
if (length(args) > 2)
{
  ...
  group_data = read.table(file=args[3], sep="\t", header=T, check.names=F, comment.char = "")
  group = as.vector(group_data[,2])
  legend_title = "Group"
...
pca=ggplot(pc,aes(PC1,PC2)) + 
  ...
  labs(x=xlab,y=ylab,title="PCA",color=legend_title) + 
...
ggsave(paste(outname,".PCA2D.pdf",sep=""),pca,width=10,height=8)
ggsave(paste(outname,".PCA2D.png",sep=""),pca,width=10,height=8)

cat("[LOG] 2D PCA plot with sample names was created :)\n")

ggsave(paste(outname,".PCA2D.nonames.pdf",sep=""),nonames_pca,width=10,height=8)
ggsave(paste(outname,".PCA2D.nonames.png",sep=""),nonames_pca,width=10,height=8)
...
draw_3d_pca = function(file,width=10,height=8,names=TRUE,legend=TRUE) {
  ...
  if (length(args) > 2 && legend == TRUE)
  {
    group_data = read.table(file=args[3], sep="\t", header=T, check.names=F, comment.char = "")
    group = group_data[,2]
    group_names = as.vector(unique(group))

    group_color = gg_color_hue(length(group_names))
    
    legend("right",group_names,inset=inset,col=group_color,pch=16,cex=1.2,title=legend_title,bty="n",xpd=TRUE,ncol=legend_ncol)
  } else if (legend == TRUE) {
    legend("right",sample_names,inset=inset,col=sample_colour,pch=16,cex=1.2,title=legend_title,bty="n",xpd=TRUE,ncol=legend_ncol)
  }
...
draw_3d_pca(file=paste(outname,".PCA3D.pdf",sep=""),names=TRUE,legend=TRUE,width=width,height=8)

#-------------------------------------
# draw 3D plot without sample names (pdf)
draw_3d_pca(file=paste(outname,".PCA3D.nonames.pdf",sep=""),names=FALSE,legend=TRUE,width=width,height=8)

cat("[LOG] 3D PCA plot (pdf) was created :)\n")

#-------------------------------------
# draw 3D plot with sample names (png)
draw_3d_pca(file=paste(outname,".PCA3D.png",sep=""),names=TRUE,legend=TRUE,width=width*100,height=800)

#-------------------------------------
# draw 3D plot without sample names (png)
draw_3d_pca(file=paste(outname,".PCA3D.nonames.png",sep=""),names=FALSE,legend=TRUE,width=width*100,height=800)
